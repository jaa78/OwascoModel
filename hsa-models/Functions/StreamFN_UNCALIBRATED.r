...
##  Previously called StreamFN_Aug2012.r

StreamflowFN	<-	function(
					rec_coef	= 0.1, 		# based on a study by Weiler in NY state
					Se_min		= 78, 		# mm
					C1			= 3.1,		# Watershed analysis
					Ia_coef		= 0.05, 		# range ~ 0.5 - 0.2
					...
					BF1			= 1, 	#  mm/day can use nearby watershed baseflow 	ConvCFS_to_mm_d(cfs,W_Area)
...
## Potential Evapotranspiration 
	PET		<-	PETfromTemp(Jday=day,Tmax_C=Tmax,Tmin_C=Tmin,AvgT=Tav,albedo=albedo,lat=latitude)*1000		## mm (Priestley-Taylor)
...
RunoffBreakdown <- function(Tp_hr, a = 4.5, HrPrcDelay = 4, numDaysReturn=5){
Tp <- Tp_hr/24
DRO <- rep(0,20)
t <- 1
...
##  Convert USGS gage streamflow (cfs) to mm/day depth

ConvCFS_to_mm_d <- function(cfs, WA, AREAunits = "mi2"){
	feetpermile <- 5280
	mmperfoot 	<- 304.8
