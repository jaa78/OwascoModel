...
##  Initial Values.  
	...
	if (is.null(YestAlbedo)){
		...
		if (NewSnow[1] > 0){ 
			Albedo[1] <- 0.98-(0.98-YestAlbedo)*exp(-4*NewSnow[i]*10)
	...
	SnowDensity[1] <- 	ifelse((startingSnowDepth_m+NewSnow[1])>0, min(450, 
		((startingSnowDensity_kg_m3+k*30*(450-startingSnowDensity_kg_m3)*exp(-DCoef[i]))*startingSnowDepth_m + NewSnowDensity[1]*NewSnow[1])/(startingSnowDepth_m+NewSnow[1])), 450)	
...
##  Snow Melt Loop	
	...
	Results<-data.frame(Date, Tmax_C, Tmin_C, precip_mm, R_m*1000, NewSnowWatEq*1000,SnowMelt*1000, NewSnow, SnowDepth, SnowWaterEq*1000, SnowTemp, Albedo, Energy, S, La, Lt, H, E, AE, cloudiness)
