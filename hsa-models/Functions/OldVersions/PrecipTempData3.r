## PrecipTempData.r
## Josephine Archibald
## Updated 2012-7-24, previous update: 2011-12-27
...
if(is.na(IthPrecip)) IthPrecip <- mean(todaysTable[grep("ITHACA", todaysTable[,6]),5])
AuburnPrecip <- mean(todaysTable[grep("Auburn", todaysTable[,6]),5])
if(is.na(AuburnPrecip)) AuburnPrecip <- mean(todaysTable[grep("AUBURN", todaysTable[,6]),5])
CortlandPrecip <- mean(todaysTable[grep("Cortland", todaysTable[,6]),5])
if(is.na(CortlandPrecip)) CortlandPrecip <- mean(todaysTable[grep("CORTLAND", todaysTable[,6]),5])
FreevillePrecip <- mean(todaysTable[grep("Freeville", todaysTable[,6]),5])
if(is.na(FreevillePrecip)) FreevillePrecip <- mean(todaysTable[grep("FREEVILLE", todaysTable[,6]),5])
...
if(is.na(AuroraPrecip)) AuroraPrecip <- mean(todaysTable[grep("AURORA", todaysTable[,6]),5])
LockePrecip<-mean(todaysTable[grep("Locke", todaysTable[,6]),5])
if(is.na(LockePrecip)) LockePrecip <- mean(todaysTable[grep("LOCKE", todaysTable[,6]),5])
SyracusePrecip<- mean(todaysTable[grep("Syracuse", todaysTable[,6]),5])
if(is.na(SyracusePrecip)) SyracusePrecip <- mean(todaysTable[grep("SYRACUSE", todaysTable[,6]),5])
MoraviaPrecip<-mean(todaysTable[grep("Moravia", todaysTable[,6]),5])
if(is.na(MoraviaPrecip)) MoraviaPrecip <- mean(todaysTable[grep("MORAVIA", todaysTable[,6]),5])
	##  Note:  Moravia added back July 24, 2012 
